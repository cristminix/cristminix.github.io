...
var KTLayoutSearch = function () {
    ...
    var processsAjax = function (search) {
        ...
        axios.get('/blog/search?query=' + searchObject.getInputElement().value)
            .then(function (response) {
                resultsElement.innerHTML = '';
                if (response == null || response.data == null || response.data.length == 0) {
                    resultsElement.innerHTML = `<div class="text-muted fs-7">No results for ( ` + searchObject.getInputElement().value + ` )</div>`;
                } else {
                    for (var i = 0; i < response.data.length; i++) {
                        resultsElement.innerHTML += `<a href="/blog/` + response.data[i].slug + `" class="d-flex text-dark text-hover-primary align-items-center mb-5"><div class="d-flex flex-column justify-content-start fw-semibold"><span class="fs-6 fw-semibold">` + response.data[i].title + `</span><div></a>`;
                    }
                }
                resultsElement.classList.remove('d-none');
                emptyElement.classList.add('d-none');
